...
public class UsersDbClient implements UsersClient {
    ...
    public UserJson createUser(String username, String password) {
        return xaTransactionTemplate.execute(() -> {
            ...
            return UserJson.fromEntity(userdataUserRepository.create(userdataUserEntity(username)), null);
    ...
    public void addIncomeInvitation(UserJson targetUser, int count) {
        if (count > 0) {
            UserdataUserEntity targetEntity = userdataUserRepository.findById(requireNonNull(targetUser.id())).orElseThrow();
                        ...
                        .add(UserJson.fromEntity(
                                requireNonNull(
                                        xaTransactionTemplate.execute(() -> {
                                            ...
                                            final UserdataUserEntity newUser = createNewUser(username, defaultPassword);
                                            userdataUserRepository.addIncomeInvitation(targetEntity, newUser);
                                            return newUser;
                                        })),
    ...
    @Nonnull
    private UserdataUserEntity createNewUser(String username, String password) {
        AuthUserEntity authUser = authUserEntity(username, password);
        authUserRepository.create(authUser);
        return userdataUserRepository.create(userdataUserEntity(username));
    }
